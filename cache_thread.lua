...
function CT:moveUp(id)
  ...
  self.used = self.used - self.memory[id].size
  self.memory[id] = nil
...
function CT:find(id)
  if self.memory[id] then
    self.saved = self.saved + self.memory[id].size
...
function CT:cache(request)
  if self.size > self.used + request.size then
    self.memory[request.id] = request
    self.memory[request.id].hits = 1
    self.downloads = self.downloads + 1
    self.used = self.used + request.size
    self.stored = self.stored + 1
    return true
  end
  return false
